...
export default function ChatInput({ onSend, onStop, isStreaming }: ChatInputProps) {
    ...
    // A provider with an empty keyPrefix (e.g. THOX) is keyless — the hosted
    // Spaces need no browser key, so "ready" must not gate on an API key.
    const requiresKey = (provider?.keyPrefix ?? 'x') !== '';
    const apiKey = getApiKey(activeProviderId);
    const ready = !requiresKey || !!apiKey;
                ...
                {voiceMode ? (
                    ...
                    <div className="relative flex flex-col bg-bg-surface border border-border rounded-xl focus-within:border-accent/50 transition-colors">
                        ...
                        <textarea
                            ...
                            placeholder={ready ? 'Type a message…' : 'Set your API key in Settings first…'}
                            rows={1}
                            disabled={!ready}
                        ...
                        <div className="flex items-center justify-between px-3 pb-2 pt-1">
                            ...
                            <div className="flex items-center gap-1.5">
                                {isStreaming ? (
                                    ...
                                    <button
                                        onClick={handleSend}
                                        disabled={(!text.trim() && attachments.length === 0) || !ready}

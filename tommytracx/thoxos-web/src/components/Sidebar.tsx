...
import { Search, Plus, Settings, Palette, Trash2, Pencil, Check, X, MessageSquare, Inbox as InboxIcon, Package, Radio } from 'lucide-react';
import { useApp } from '../context/AppContext';
import { useConversations, createConversation, deleteConversation, renameConversation } from '../lib/hooks';
import { unreadInboxCount } from '../lib/fabric';
...
export default function Sidebar() {
    const { activeConversationId, setActiveConversationId, sidebarOpen, setSidebarOpen, activeModal, setActiveModal, activeProviderId, activeModelId } = useApp();
    ...
    // Inbox badge — refreshed when the panel closes so a read item clears the count.
    useEffect(() => {
        unreadInboxCount().then(setUnread).catch(() => undefined);
    }, [activeModal]);
    ...
    return (
        ...
        `}
            >
                ...
                {/* Workspace — THOX CX Fabric surfaces + the live serving view */}
                <div className="px-3 pt-2 pb-1 border-t border-border grid grid-cols-3 gap-1.5">
                    <button
                        onClick={() => setActiveModal('inbox')}
                        className="relative flex flex-col items-center gap-1 py-2 rounded-lg text-text-secondary hover:text-text-primary hover:bg-bg-surface-hover transition-colors"
                        title="Inbox — completed deliverables"
                    >
                        <InboxIcon size={15} />
                        <span className="text-[10px]">Inbox</span>
                        {unread > 0 && (
                            <span className="absolute top-1 right-2 min-w-[15px] h-[15px] px-1 rounded-full bg-accent text-[9px] font-semibold text-white flex items-center justify-center">
                                {unread > 9 ? '9+' : unread}
                            </span>
                        )}
                    </button>
                    <button
                        ...
                        className="flex flex-col items-center gap-1 py-2 rounded-lg text-text-secondary hover:text-text-primary hover:bg-bg-surface-hover transition-colors"
                        title="Artifacts library"
                    >
                        <Package size={15} />
                        <span className="text-[10px]">Artifacts</span>
                    </button>
                    <button
                        onClick={() => setActiveModal('fleet')}
                        className="flex flex-col items-center gap-1 py-2 rounded-lg text-text-secondary hover:text-text-primary hover:bg-bg-surface-hover transition-colors"
                        title="ThoxRoute fleet status"
                    >
                        <Radio size={15} />

...
import { applyThoxRouteRegistry } from '../lib/providers';
...
const GLOBAL_CANVAS_KEY = 'thoxos_global_canvas_id';
...
interface AppContextType {
    ...
    /** ThoxRoute serving status: which models this deployment can actually reach. */
    routeStatus: ThoxRouteStatus | null;
}
...
export function AppProvider({ children }: { children: ReactNode }) {
    ...
    const [routeStatus, setRouteStatus] = useState<ThoxRouteStatus | null>(null);
    ...
    // Load the ThoxRoute serving registry once at boot. This is what makes the model list
    // config-driven: a model whose endpoint env var is now set shows up here without a rebuild.
    // Failure is non-fatal — the built-in THOX defaults remain usable.
    useEffect(() => {
        let cancelled = false;
        getThoxRouteStatus()
            .then((status) => {
                if (cancelled) return;
                applyThoxRouteRegistry(status.models);

...
export function stopPlayback() {
    ...
    // Also halt any in-flight browser speech synthesis.
...
/** Whether the free browser speech-synthesis fallback is available. */
export function browserTTSSupported(): boolean {
    return typeof window !== 'undefined' && 'speechSynthesis' in window;
}

/**
 * Speak text using the browser's built-in speechSynthesis. Used as the free
 * fallback whenever no cloud TTS key is configured. Honours stopPlayback().
 */
export async function speakBrowser(
    ...
    // speechSynthesis can choke on very long strings — chunk conservatively.
    ...
    for (const chunk of chunks) {
        ...
        await new Promise<void>((resolve, reject) => {
            ...
            utter.onend = () => resolve();
            utter.onerror = () => {
                // A cancel() surfaces as onerror ('canceled'/'interrupted') —
                // treat that as a clean stop rather than a failure.
                if (playbackCancelled) resolve();
                else reject(new Error('Speech synthesis error'));
            };
            synth.speak(utter);
        });
    }
...
/**
 * Strip Markdown and fenced code so TTS reads clean prose. Shared helper so the
 * full-duplex voice panel and any other caller stay consistent.
 */
export function stripMarkdownForSpeech(content: string): string {
    ...
    for (const line of lines) {
        ...
        if (!inCodeBlock) textLines.push(line);
    }

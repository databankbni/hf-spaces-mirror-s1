"""
utils/tzcc.py — offline IANA-timezone -> ISO-3166 alpha-2 country map.

Generated from the public-domain tz database (zone1970.tab / zone.tab). Lets the
app derive an approximate visitor country from the browser's timezone, with no IP
lookup and no third-party request. Timezone -> country is coarse (one representative
country per zone) but is unaffected by the hosting proxy, so it reflects the real
visitor rather than the datacenter.
"""
...
_TZ2CC = {
    ...
    'America/Ciudad_Juarez': 'MX', 'America/Costa_Rica': 'CR', 'America/Coyhaique': 'CL',
    ...
    'Asia/Brunei': 'BN', 'Asia/Calcutta': 'IN', 'Asia/Chita': 'RU', 'Asia/Colombo': 'LK',
    ...
    'Asia/Istanbul': 'TR', 'Asia/Jakarta': 'ID', 'Asia/Jayapura': 'ID', 'Asia/Jerusalem': 'IL',
    'Asia/Kabul': 'AF', 'Asia/Kamchatka': 'RU', 'Asia/Karachi': 'PK', 'Asia/Kathmandu': 'NP',
    'Asia/Katmandu': 'NP', 'Asia/Khandyga': 'RU', 'Asia/Kolkata': 'IN',
    ...
    'Australia/Sydney': 'AU', 'Canada/Eastern': 'CA', 'Canada/Pacific': 'CA',
    ...
    'Europe/Minsk': 'BY', 'Europe/Monaco': 'MC', 'Europe/Moscow': 'RU', 'Europe/Oslo': 'NO',
    ...
    'Europe/Zagreb': 'HR', 'Europe/Zurich': 'CH', 'Indian/Antananarivo': 'MG',
    ...
    'Pacific/Tongatapu': 'TO', 'Pacific/Wake': 'UM', 'Pacific/Wallis': 'WF', 'US/Alaska': 'US',
    'US/Arizona': 'US', 'US/Central': 'US', 'US/Eastern': 'US', 'US/Hawaii': 'US',
    'US/Mountain': 'US', 'US/Pacific': 'US',
}


def country_from_tz(tz):
    """2-letter ISO country for an IANA timezone string, or "" if unknown."""
    try:
        return _TZ2CC.get((tz or '').strip(), '')
    except Exception:
        return ''

"""Past-report D2 (Roof coverings) generate with property_type=house.

Runs two generations back-to-back and writes clean plaintext under
docs/past-d2-house-runs/ plus COMPARE_latest_two.txt.
"""
...
OUT_DIR = _ROOT / "docs" / "past-d2-house-runs"
RUNS = int(os.environ.get("PAST_HOUSE_RUNS", "2"))

NOTES = """The roof is a pitched with interlocking concrete tiles. The roof has been replaced. The date of this replacement could not be confirmed. Parapet walls have been rendered and the render showed cracks. Distening was observed to the parapet walls below coping stones. Flashings at the abutments of parapet walls and roof are lead. Some sections have been lifted, creating gaps. The mortar bed into the ridge tiles is deteriorated. There is some moss and lichen growth. The main roof covering is serviceable order and no deflection."""
...
def _write_compare_latest_two() -> Path | None:
    recent = sorted(
        (p for p in OUT_DIR.glob("past-d2-house-*/generated.txt") if p.is_file()),

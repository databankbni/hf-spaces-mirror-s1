...
SCHEMA_PATH = Path(__file__).resolve().parent.parent / "schemas" / "world_bank.json"


class WorldBankFormFactory:
    @classmethod
    def create_form(cls, dataset_id: str, data: dict = None) -> forms.Form:
        ...
        form_class = type(f"WorldBank_{dataset_id.replace('-', '_').title()}_Form", (forms.Form,), fields)

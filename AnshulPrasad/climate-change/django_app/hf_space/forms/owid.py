...
SCHEMA_PATH = Path(__file__).resolve().parent.parent / "schemas" / "owid.json"


class OWIDFormFactory:
    @classmethod
    def create_form(cls, dataset_id: str, data: dict = None) -> forms.Form:
        ...
        form_class = type(f"OWID_{dataset_id.replace('-', '_').title()}_Form", (forms.Form,), fields)

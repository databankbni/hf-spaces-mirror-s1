"""
Data models for the PersonalAssistantBench RL Environment.

Wraps PersonalAssistantBench (an on-device iOS assistant benchmark: 14 tasks over the real
...
class TrajectoryEvent(BaseModel):
    """One recorded event, mirroring PersonalAssistantBench's trajectory.jsonl vocabulary."""

    event: str  # reset | agent_start | function_call | tool_result | agent_done | verify_reminders
    fields: Dict[str, Any] = Field(default_factory=dict)


class PersonalAssistantBenchAction(Action):
...
class PersonalAssistantBenchObservation(Observation):
...
class PersonalAssistantBenchState(State):
    """Internal env state."""

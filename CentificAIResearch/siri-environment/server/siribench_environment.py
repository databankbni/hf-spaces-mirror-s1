"""
SiriBench RL Environment — the neutral SiriBench agent loop as an OpenEnv env.
...
(or on the step limit). Every event is recorded in SiriBench's trajectory
...
from siribench_env.models import (
    EpisodeStatus,
    Message,
    SiriBenchAction,
    SiriBenchObservation,
    SiriBenchState,
    ToolName,
    TrajectoryEvent,
)
...
class SiriBenchEnvironment(Environment):
    """OpenEnv environment wrapping the 14-task SiriBench suite."""
    ...
    def step(self, action: SiriBenchAction, timeout_s: Optional[float] = None, **kwargs: Any) -> SiriBenchObservation:  # type: ignore[override]
        s = self._state
    ...
    def _handle_respond(self, text: str) -> SiriBenchObservation:
        s = self._state
    ...
    @property
    def state(self) -> SiriBenchState:  # type: ignore[override]
        return self._state

...
def venv(make_env):
    return make_env(
        ...
        REQUIRE_TRACE_FOR_RESULTS="false",
    )
...
def post_result(env, *, score=200.0, artifacts="hf://buckets/test-org/test-agent-1/submissions/v1/", extra_fm=None):
    env.hub.seed("results/run.xlsx", "fake spreadsheet bytes", bucket=SCRATCH)
    fields = {"score": score, "method": "vllm-fp8", "status": "agent-run", "description": "fast"}
    ...
    r = env.client.post(
        "/v1/results",
        json={"source": "hf://buckets/test-org/test-agent-1/results/run.xlsx", "fields": fields},
    )
...
def test_disabled_verifier_is_inert(make_env):

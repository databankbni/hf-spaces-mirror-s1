...
def scatter_add(src, index, dim=0, dim_size=None):
    ...
    if src.dim() == 1:
        ...
        index_exp = index.view(-1, 1).expand_as(src) if (dim == 0 and src.dim() == 2) else index
...
def scatter_max(src, index, dim=0, dim_size=None):
    ...
    out_shape = list(src.shape)
    out_shape[dim] = dim_size
    out = torch.full(out_shape, fill_value=float('-inf'), dtype=src.dtype, device=src.device)
    if src.dim() == 1:
        out = out.scatter_reduce(0, index, src, reduce='amax', include_self=False)
    else:
        index_exp = index.view(-1, 1).expand_as(src) if (dim == 0 and src.dim() == 2) else index
        out = out.scatter_reduce(dim, index_exp, src, reduce='amax', include_self=False)
    return out, None

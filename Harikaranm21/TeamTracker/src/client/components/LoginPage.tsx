...
import {
  Box, Flex, Text, Button, TextField, Callout
...
export function LoginPage({ onSuccess }: { onSuccess?: () => void } = {}): React.ReactElement {
    ...
    try {
      if (mode === 'login') {
        ...
        if (result.user.role === 'admin') {
          ...
          setSuccessMsg(result.message ?? 'Registration successful. Await admin approval.');
          reset('login');
        }
      }
    } catch (err) {
    ...
    >
      ...
      >
        ...
        <Flex align="center" gap="2" mb="6">
          <SquaresFour size={26} weight="fill" style={{ color: 'var(--accent-9)' }} />
          <Text size="5" weight="bold">TeamTracker</Text>
        </Flex>
        ...
        <form onSubmit={handleSubmit}>
          <Flex direction="column" gap="3">
            <Box>
              <Text as="label" size="2" weight="medium" htmlFor="auth-username">Username</Text>
              <TextField.Root
                id="auth-username"
                mt="1"
            ...
            {mode === 'register' && (
              <Box>
                <Text as="label" size="2" weight="medium" htmlFor="auth-email">Email</Text>
                <TextField.Root
                  id="auth-email"
                  mt="1"
                  type="email"
            ...
            <Box>
              <Text as="label" size="2" weight="medium" htmlFor="auth-password">Password</Text>
              <TextField.Root
                id="auth-password"
                mt="1"
                type="password"

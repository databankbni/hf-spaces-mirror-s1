...
export function TaskDialog({
  ...
  useEffect(() => {
    if (open) {
      ...
      setAssigneeId(
        viewerMode
          ? String(members.find((m) => m.email === currentUser.email)?.id ?? "")
          : task?.assignee_id
  ...
  const handleSave = async (): Promise<void> => {
    ...
    try {
      await onSave({
        ...
        status,
        priority,
        assignee_id: viewerMode
          ? (members.find((m) => m.email === currentUser.email)?.id ?? null)
          : assigneeId === "none"
            ? null
            : Number(assigneeId),
    ...
    <Dialog.Root open={open} onOpenChange={(o) => !o && onClose()}>
      <Dialog.Content style={{ maxWidth: 520 }}>
            ...
            <Box style={{ flex: 1 }}>
              ...
              >
                ...
                <Select.Content>
                  ...
                  {members
                    .filter((m) => !viewerMode || m.email === currentUser.email)
                    .map((m) => (
        ...
        <Flex gap="3" mt="4" justify="end">
          <Dialog.Close>
            <Button variant="soft" color="gray" title="Cancel">
              Cancel
            </Button>
          </Dialog.Close>
          <Button

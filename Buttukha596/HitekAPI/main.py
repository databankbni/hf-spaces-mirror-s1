...
# --- SECURITY ENGINE ---
security = HTTPBearer()
# This reads from Hugging Face Repository Secrets securely
HF_SECRET_TOKEN = os.getenv("API_BEARER_TOKEN")

def verify_token(credentials: HTTPAuthorizationCredentials = Depends(security)):
    if not HF_SECRET_TOKEN:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Server configuration error: Token missing from environment",
            headers={"WWW-Authenticate": "Bearer"},
        )
    if credentials.credentials != HF_SECRET_TOKEN:
        raise HTTPException(
...
# --- GLOBAL REJECTION HANDLER (Strict Paths Only) ---
    ...
    if exc.status_code == 404:
        return JSONResponse(
            ...
            content={
                ...
                "Developer": "@Real_BITTU"
            }
        )
    return JSONResponse(
        status_code=exc.status_code,
        content={"detail": exc.detail, "Developer": "@Real_BITTU"}
    )

# Unprotected route for the status landing page
@app.get("/", response_class=HTMLResponse)
def root_landing_page():
    return HTMLResponse(content=LANDING_PAGE_HTML, status_code=200)

# --- STRICT API ENDPOINT (Now Protected by Bearer Token) ---
@app.get("/FetchData", dependencies=[Depends(verify_token)])
def fetch_data(Number: str = Query(None)):
    # Enforce precise formatting
    ...
    primary_url = f"https://huggingface.co/datasets/Buttukha596/hitek-data/resolve/main/final_master_shard_{last_digit}.parquet"
    alt_url = f"https://huggingface.co/datasets/Buttukha596/hitek-data/resolve/main/alt_master_shard_{last_digit}.parquet"
    
    try:
        # One parallel query that grabs both at once, tagging them for the JSON parser
        query = f"""
        ...
        # Instantly sort the combined results into their JSON categories
        for row in raw_results:
            rec_type = row.pop('_record_type') # Strip out the temporary hidden column
            if rec_type == 'Main':

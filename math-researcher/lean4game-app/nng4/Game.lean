...
Introduction
"
# Welcome to the Natural Number Game, {name}!

You're about to build the natural numbers `{0,1,2,3,4,...}` from scratch.
First you'll prove that `2 + 2 = 4`. Then you'll prove that `x + y = y + x`.
By the end, you'll have the real proof skills to take on Fermat's Last Theorem.
You'll do all of this by solving levels of a real theorem-proving puzzle called Lean.

# Read this.

Take your time with the hint text on each level -- players who read it get the
most out of this game.

Tap this chapter in the list to start.

## More

Open your Profile tab for game info, credits, and ways to connect with the Lean
community.
"

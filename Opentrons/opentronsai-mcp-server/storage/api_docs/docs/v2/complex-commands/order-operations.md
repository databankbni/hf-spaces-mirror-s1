...
In a `transfer()`, aspiration and dispensing are the only required actions. You can enable or disable all of the other actions with [complex liquid handling parameters](parameters.md). A `transfer()` command designed to perform every possible action will proceed in this order:

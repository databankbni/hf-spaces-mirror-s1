...
def save_selected_models(selected_models):
    ...
    try:
        ...
        if saved_files:
              files_list = "\n".join([
                    f"<a href='file/{file}' download style='text-decoration: none;'>• {os.path.basename(file)}</a>"
              ...
              return (
                  ...
                  <div style="background-color: #e8f5e8; padding: 15px; border-radius: 8px; border-left: 5px solid #28a745;">
                      ...
                      <ul style="font-size: 13px; color: #155724;">
                          {''.join(f"<li>{os.path.basename(f)}</li>" for f in saved_files)}
                      </ul>
        ...
        with gr.Column(scale=2):
            ...
            with gr.Tabs():
                ...
                with gr.TabItem("💾 Save models"):
                    ...
                    save_status = gr.HTML("")
                    downloadables = gr.File(label="Download models", visible=False)

...
st.set_page_config(
    page_title="사모신용 카나리아 모니터링 (Preview2 — KODEX 스타일)",
    layout="wide",
...
st.markdown(
    ...
    <style>
        ...
        /* ─── KODEX 스타일 hero — 블루 그라데이션 배너 ─── */
        .pcc-header {
            display: flex; align-items: center; justify-content: space-between;
            gap: 16px;
            padding: 28px 32px;
            margin: 0 0 18px 0;
            background: linear-gradient(135deg, #2D55E0 0%, #4F7BFF 60%, #6E97FF 100%);
            border-radius: 24px;
            box-shadow: 0 8px 24px rgba(45, 85, 224, 0.22);
            color: #ffffff;
        }
        ...
        /* ─── KODEX 스타일 — 모든 bordered 컨테이너 (box_hy / box_returns / box_news / box_sec / 좌·우 summary 카드)
              radius 22px + soft shadow + 흰 배경 + 더 큰 패딩 ─── */
        ...
        /* 박스 안쪽 콘텐츠 패딩 — 기존 14px → 20px 로 여유 */
        ...
        .st-key-returns_period_box button.st-emotion-cache-pillsActive {
            /* KODEX 스타일 — 흰 배경 + 진한 블루 테두리·글자 + 살짝 그림자 */
...
def render_header(
    ...
    # KODEX 스타일 — hero 배너 자체가 시각적 구분이 되므로 divider 생략
...
def _risk_trend(
    ...
    # history 에서 (date, composite_score) 쌍을 추출 — 최근 30개만 표시
    dates: list[date] = []
    series: list[float] = []
    if history:
        for h in history:
    ...
    fig.add_trace(go.Scatter(
        ...
        marker=dict(symbol="circle", size=26, color="rgba(255,255,255,0.4)"),
        hoverinfo="skip", showlegend=False,
    ))
    # 중간 글로우 (더 진함)
    fig.add_trace(go.Scatter(
        x=dates, y=series, mode="markers",
        marker=dict(symbol="circle", size=18, color="rgba(255,255,255,0.7)"),
        hoverinfo="skip", showlegend=False,
    ))

    # 메인 라인 + 마커 — KODEX 스타일 (밝은 코랄 #EF4444 + 옅은 핑크 fill)
    fig.add_trace(go.Scatter(
        x=dates, y=series,
        mode="lines+markers",
        line=dict(color="#EF4444", width=2.2, shape="spline", smoothing=1.0),
        marker=dict(
            symbol="circle",
            size=9,
            color="#EF4444",
        ...
        fill="tozeroy",
        fillcolor="rgba(252, 165, 165, 0.35)",  # 파스텔 핑크 — KODEX 톤
...
def render_summary(price_df: pd.DataFrame | None = None) -> None:
    ...
    with left:
        with st.container(border=True, height=_SUMMARY_CARD_HEIGHT):
            ...
            # KODEX 스타일 — 큰 pill + 점수 강조 (3rem) + soft shadow
            st.markdown(
                f'<div style="text-align:center; margin-top:-4px;">'
                f'<span style="display:inline-block; background:{color}; color:white; '
                f'padding:8px 36px; border-radius:999px; font-weight:700; font-size:1.05rem; '
                f'box-shadow: 0 4px 12px {color}55;">'
                f'{level}</span>'
                f'<div style="margin-top:14px; color:#0f172a; font-weight:700; font-size:2.2rem; line-height:1;">'
                f'{int(round(score))}'
                f'<span style="color:#94a3b8; font-size:1.1rem; font-weight:500;"> / 100</span>'
                f'</div>'
                f'</div>',
                unsafe_allow_html=True,
            )
    ...
    with right:
        with st.container(border=True, height=_SUMMARY_CARD_HEIGHT):
            ...
            st.markdown(
                """
                <div style="display:flex; align-items:baseline; gap:1px; margin-bottom:0;">
                    <h5 style="margin:0; font-size:1.1rem;">
            ...
            if composite_insight:
                st.markdown(
                    f'<div style="font-size:1.1rem; color:#1e293b; line-height:1.65; '
...
def _news_tags(row: pd.Series, max_tags: int = 2) -> list[str]:
    """matched_tags 에서 짧은 토큰 추출. primary_hits / search_keyword 는 제외 —
    사모대출 / private credit 은 기사 모두 포함되는 필수 키워드라 태그로는 변별력이 없음.
    """
    tags: list[str] = []

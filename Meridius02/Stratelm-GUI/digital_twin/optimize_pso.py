...
V_MIN = 15.0 # km/h
V_MAX = 45.0 # km/h -- see optimize_ga.py's comment: briefly lowered to 35 chasing a
                     # two-motor infeasibility that was actually a simulate.py bug
                     # (dist_to_stop_m self-matching a stop, starving the accel motor
                     # on relaunch), now fixed -- full range is feasible again.
...
def optimize_strategy(scenario_name="typical_january", motor_name=config.DEFAULT_MOTOR_NAME, fc_name=config.DEFAULT_FC_NAME,
                       ...
                       segment_out_path=None, telemetry_out_path=None):
    ...
    # Using PSO instead of GA
    algorithm = PSO(pop_size=pop_size)
    
    print(f"Starting PSO optimization ({pop_size} pop, {n_gen} gen)...")
    res = minimize(
        problem,
        algorithm,
        ("n_gen", n_gen),
    ...
    gene_table.to_csv(segment_out_path or "data/pso_segment_targets.csv", index=False)
    print("\nPer-segment gas/glide targets (from PSO) -> data/pso_segment_targets.csv:")
    ...
    best_telemetry.to_csv(telemetry_out_path or "data/simulated_telemetry_pso.csv", index=False)
